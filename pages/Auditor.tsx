        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {recipeIdeas.length === 0 ? (
            ...
            recipeIdeas.map((recipe, index) => (
              <div key={recipe.id || `recipe-${index}`} className="border border-gray-200 rounded-xl p-4 hover:border-primary/50 transition-colors bg-white shadow-sm">
                <h3 className="font-bold text-gray-900 mb-2">{recipe.title}</h3>
                <div className="flex flex-wrap gap-2 mb-3">
                  {recipe.tags.map((tag, idx) => (
                    <span key={`${tag}-${idx}`} className="px-2 py-0.5 bg-orange-100 text-orange-800 text-xs font-medium rounded-md flex items-center gap-1 whitespace-nowrap">
                  ...
                  <div>
                    ...
                    <div className="w-full min-h-[42px] border border-gray-300 rounded-lg p-1.5 flex flex-wrap gap-1.5 items-center bg-white">
                      {currentSettings.dietaryOptions.map((option, idx) => (
                        <span key={`${option}-${idx}`} className="px-2.5 py-1 bg-emerald-100 text-emerald-800 text-xs font-medium rounded-md flex items-center gap-1">
                          {option}
                          <button onClick={() => handleDietaryOptionRemove(option)} className="hover:text-emerald-900 ml-0.5">
                            &times;
                          </button>
                        </span>
                      ))}
                  ...
                  <div>
                    ...
                    <div className="w-full min-h-[42px] border border-gray-300 rounded-lg p-1.5 flex flex-wrap gap-1.5 items-center bg-white">
                      {currentSettings.allergies.map((allergy, idx) => (
                        <span key={`${allergy}-${idx}`} className="px-2.5 py-1 bg-emerald-100 text-emerald-800 text-xs font-medium rounded-md flex items-center gap-1">
                          {allergy}
                          <button onClick={() => handleAllergyRemove(allergy)} className="hover:text-emerald-900 ml-0.5">
                            &times;
                          </button>
                        </span>
                      ))}
                    ...
                    <div className="w-full border border-gray-300 rounded-lg p-3 bg-gray-50 text-sm text-gray-500 min-h-[46px] flex flex-wrap gap-2">
                      {currentSettings.ingredientsToAvoid.length === 0 ? (
                        ...
                        currentSettings.ingredientsToAvoid.map((ingredient, idx) => (
                          <span key={`${ingredient}-${idx}`} className="px-2.5 py-1 bg-gray-200 text-gray-700 text-xs font-medium rounded-md flex items-center gap-1">
                            {ingredient}
                            <button onClick={() => handleIngredientRemove(ingredient)} className="hover:text-gray-900 ml-0.5">
                              &times;
                            </button>
                          </span>
                        ))
